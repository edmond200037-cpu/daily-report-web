...
export async function listMemoryCandidates(): Promise<MemoryCandidate[]> { const [sites, allTrades, vendors, tasks, locations, allTypes, items] = await Promise.all([listMemories('sites', undefined, 'candidate'), listMemories('trades', undefined, 'all'), listMemories('vendors', undefined, 'candidate'), listMemories('tasks', undefined, 'candidate'), listMemories('locations', undefined, 'candidate'), listMaterialTypes('all'), listMaterialMemory('candidate')]); const trades = allTrades.filter((row) => row.status === 'candidate'); const types = allTypes.filter((row) => row.status === 'candidate'); const tradeById = new Map(allTrades.map((row) => [row.id, row])); const typeById = new Map(allTypes.map((row) => [row.id, row])); const named = (kind: Extract<MemoryKind, 'sites' | 'trades' | 'vendors' | 'tasks' | 'locations'>, values: NamedMemory[]) => values.map((row) => { const parent = row.tradeTypeId ? tradeById.get(row.tradeTypeId) : undefined; return { key: memoryCandidateKey(kind, row.id), kind, id: row.id, name: row.name, parentName: parent?.name, parentKey: parent?.status === 'candidate' ? memoryCandidateKey('trades', parent.id) : undefined, usageCount: row.usageCount, finalizedUsageCount: row.finalizedUsageCount ?? 0, lastUsedAt: row.lastUsedAt }; }); return [...named('sites', sites), ...named('trades', trades), ...named('vendors', vendors), ...named('tasks', tasks), ...named('locations', locations), ...types.map((row) => ({ key: memoryCandidateKey('material-types', row.id), kind: 'material-types' as const, id: row.id, name: row.name, usageCount: row.usageCount, finalizedUsageCount: row.finalizedUsageCount ?? 0, lastUsedAt: row.lastUsedAt })), ...items.map((row) => { const parent = typeById.get(row.materialTypeId); return { key: memoryCandidateKey('material-items', row.id), kind: 'material-items' as const, id: row.id, name: row.value, parentName: parent?.name, parentKey: parent?.status === 'candidate' ? memoryCandidateKey('material-types', parent.id) : undefined, fieldType: row.fieldType, usageCount: row.usageCount, finalizedUsageCount: row.finalizedUsageCount ?? 0, lastUsedAt: row.lastUsedAt }; })]; }
export async function confirmMemoryCandidates(keys: MemoryCandidateKey[]): Promise<void> { const database = await db(); try { const stores = ['sites', 'trade_types', 'trade_vendors', 'trade_tasks', 'location_memories', 'material_types', 'material_memory_items']; const tx = database.transaction(stores, 'readwrite'); const lookup: Record<MemoryKind, string> = { sites: 'sites', trades: 'trade_types', vendors: 'trade_vendors', tasks: 'trade_tasks', locations: 'location_memories', 'material-types': 'material_types', 'material-items': 'material_memory_items' }; for (const key of keys) { const separator = key.indexOf(':'); const kind = key.slice(0, separator) as MemoryKind; const id = key.slice(separator + 1); const store = lookup[kind]; if (!store || !id) continue; const row = await request(tx.objectStore(store).get(id)) as Record<string, unknown> | undefined; if (row) tx.objectStore(store).put({ ...row, status: 'confirmed', manuallyConfirmed: true, updatedAt: now() }); } await txDone(tx); } finally { database.close(); } }
...
function applyDelete(draft: DailyReportV3, kind: string, id: string, name?: string): void { const same = (linkedId: string | null | undefined, snapshot: string | undefined) => linkedId === id || Boolean(name && normalizeName(snapshot ?? '') === normalizeName(name)); if (kind === 'sites' && same(draft.siteId, draft.siteNameSnapshot)) { draft.siteId = null; draft.siteNameSnapshot = ''; } if (kind === 'trades') { draft.tradeSections = draft.tradeSections.filter((trade) => !same(trade.tradeTypeId, trade.tradeNameSnapshot)); draft.contacts = draft.contacts.filter((contact) => !same(contact.tradeTypeId, contact.tradeNameSnapshot)); } if (kind === 'vendors') { draft.tradeSections = draft.tradeSections.filter((trade) => !same(trade.vendorId, trade.vendorNameSnapshot)); draft.contacts = draft.contacts.filter((contact) => !same(contact.vendorId, contact.vendorNameSnapshot)); } draft.tradeSections.forEach((trade) => { if (kind === 'tasks') { trade.workItems = trade.workItems.filter((work) => !same(work.taskId, work.taskTextSnapshot)); trade.workItems.forEach((work, index) => work.sortOrder = index); } if (kind === 'locations') trade.workItems.forEach((work) => { if (same(work.locationId, work.locationTextSnapshot)) { work.locationId = null; work.locationTextSnapshot = ''; trade.status = 'draft'; } }); }); if (kind === 'tasks' && name) { draft.contacts.forEach((contact) => { contact.items = contact.items.filter((item) => normalizeName(item.content) !== normalizeName(name)); contact.items.forEach((item, index) => item.sortOrder = index); }); draft.contacts = draft.contacts.filter((contact) => contact.items.length); } draft.tradeSections.forEach((trade, index) => trade.sortOrder = index); draft.contacts.forEach((contact, index) => contact.sortOrder = index); }
